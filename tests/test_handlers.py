...
from csuibot.handlers import (help, zodiac, shio, is_palindrome, loremipsum,
                              ...
                              add_question, set_question, set_answer, set_correct_answer,
                              change_answer, change_answer_next, start_zonk, try_answer,
                              next_question)
...
def test_start_zonk_in_private(mocker):
    error_msg = "Perintah ini hanya bisa di group chat."
    mocked_reply_to = mocker.patch('csuibot.handlers.bot.reply_to')
    mocker.patch('csuibot.handlers.start_zonk', return_value=error_msg)
    mock_message = Mock(text='start zonk')
    mock_message.chat.id = 1
    mock_message.chat.type = "private"

    start_zonk(mock_message)

    args, _ = mocked_reply_to.call_args
    assert args[1] == error_msg


def test_start_zonk_already_started(mocker):
    error_msg = "group already started the games"
    mocked_reply_to = mocker.patch('csuibot.handlers.bot.reply_to')
    mocker.patch('csuibot.handlers.play_trivia_game', return_value=error_msg)
    mock_message = Mock(text='start zonk')
...
def test_start_zonk_success(mocker):
    exp_val = {"answer": ["1", "2", "3", "4"], "question": "1+1", "correct": "2", "question_id": 0}
    exp_res = "1+1\n"
    mocked_reply_to = mocker.patch('csuibot.handlers.bot.reply_to')
    mocker.patch('csuibot.handlers.play_trivia_game', return_value=exp_val)
    mock_message = Mock(text='start zonk')

    start_zonk(mock_message)

    args, _ = mocked_reply_to.call_args
    assert args[1] == exp_res


def test_add_question(mocker):
    exp_msg = "write your question!!!"
    mocked_reply_to = mocker.patch('csuibot.handlers.bot.reply_to')
    mocker.patch('csuibot.handlers.add_question', return_value=exp_msg)
    mock_message = Mock(text='/add_question')

    add_question(mock_message)

    args, _ = mocked_reply_to.call_args
    assert args[1] == exp_msg


def test_change_answer(mocker):
    exp_msg = "Choose question you would like to change the answer"
    mocked_reply_to = mocker.patch('csuibot.handlers.bot.reply_to')
    mocker.patch('csuibot.handlers.change_answer', return_value=exp_msg)
    mock_message = Mock(text='/change_answer')

    change_answer(mock_message)

    args, _ = mocked_reply_to.call_args
    assert args[1] == exp_msg


def test_stop_zonk_in_private(mocker):
    error_msg = "Perintah ini hanya bisa di group chat."
    mocked_reply_to = mocker.patch('csuibot.handlers.bot.reply_to')
    mocker.patch('csuibot.handlers.stop_zonk', return_value=error_msg)
    mock_message = Mock(text='stop_zonk')
    mock_message.chat.id = 1
    mock_message.chat.type = "private"

    stop_zonk(mock_message)

    args, _ = mocked_reply_to.call_args
    assert args[1] == error_msg


def test_stop_zonk_game_not_started(mocker):
    error_msg = "Your group hasn't started the game"
    mocked_reply_to = mocker.patch('csuibot.handlers.bot.reply_to')
    mocker.patch('csuibot.handlers.stop_zonk', return_value=error_msg)
    mocker.patch('csuibot.handlers.stop_trivia_game', return_value=error_msg)
    mock_message = Mock(text='stop_zonk')
    mock_message.chat.id = 1
...
def test_stop_zonk_success(mocker):
    error_msg = "Leaderboard game barusan: \n"
    mocked_reply_to = mocker.patch('csuibot.handlers.bot.reply_to')
    mocker.patch('csuibot.handlers.stop_zonk', return_value=error_msg)
    mocker.patch('csuibot.handlers.stop_trivia_game', return_value="game stopped")
    mock_message = Mock(text='stop_zonk')
    mock_message.chat.id = 1
...
def test_next_question_in_private(mocker):
    error_msg = "Perintah ini hanya bisa di group chat."
    mocked_reply_to = mocker.patch('csuibot.handlers.bot.reply_to')
    mocker.patch('csuibot.handlers.next_question', return_value=error_msg)
    mock_message = Mock(text='next question')
    mock_message.chat.id = 1
    mock_message.chat.type = "private"

    next_question(mock_message)

    args, _ = mocked_reply_to.call_args
    assert args[1] == error_msg


def test_next_question_success(mocker):
    error_msg = "Skip soal. Soal selanjutnya ,1+1\n"
    exp_val = {"answer": ["1", "2", "3", "4"], "question": "1+1", "correct": "2", "question_id": 0}
    mocked_reply_to = mocker.patch('csuibot.handlers.bot.reply_to')
    mocker.patch('csuibot.handlers.next_question', return_value=error_msg)
    mocker.patch('csuibot.handlers.req_next_question', return_value=exp_val)
    mock_message = Mock(text='next question')
    mock_message.chat.id = 1
...
def test_set_question(mocker):
    error_msg = "write answers. There must be 4 answers separated by newline"
    mocked_reply_to = mocker.patch('csuibot.handlers.bot.reply_to')
    mocker.patch('csuibot.handlers.set_question', return_value=error_msg)
    mock_message = Mock(text='A, B, ?')

    set_question(mock_message)

    args, _ = mocked_reply_to.call_args
    assert args[1] == error_msg


def test_set_answer(mocker):
    error_msg = "Choose correct answer:"
    mocked_reply_to = mocker.patch('csuibot.handlers.bot.reply_to')
    mocker.patch('csuibot.handlers.set_answer', return_value=error_msg)
    mocker.patch('csuibot.handlers.set_answers', return_value="success")
    mock_message = Mock(text="A\nB\nC\nD\n")

    set_answer(mock_message)

    args, _ = mocked_reply_to.call_args
    assert args[1] == error_msg


def test_set_correct_answer(mocker):
    error_msg = "success"
    mocked_reply_to = mocker.patch('csuibot.handlers.bot.reply_to')
    mocker.patch('csuibot.handlers.set_correct_answer', return_value=error_msg)
    mocker.patch('csuibot.handlers.set_correct', return_value=error_msg)
    mock_message = Mock(text="C")

    set_correct_answer(mock_message)

    args, _ = mocked_reply_to.call_args
    assert args[1] == error_msg


def test_set_answert_step1(mocker):
    error_msg = "choose correct answer:"
    mocked_reply_to = mocker.patch('csuibot.handlers.bot.reply_to')
    mocker.patch('csuibot.handlers.change_answer_next', return_value=error_msg)
    mocker.patch('csuibot.handlers.get_answers', return_value="A\nB\nC\nD\n")
    mock_message = Mock(text="5. A, B, ?")

    change_answer_step1(mock_message)

    args, _ = mocked_reply_to.call_args
    assert args[1] == error_msg


def test_try_answer_invalid_command(mocker):
    error_msg = ""
    mocked_reply_to = mocker.patch('csuibot.handlers.bot.reply_to')
    mocker.patch('csuibot.handlers.try_answer', return_value=error_msg)
    mocker.patch('csuibot.handlers.judge_answer', return_value=error_msg)
    mock_message = Mock(text="start zonk")

    try_answer(mock_message)

    assert mocked_reply_to.call_args is None


def test_try_answer_no_guess_left(mocker):
    error_msg = "kesempatan guess telah habis"
    mocked_reply_to = mocker.patch('csuibot.handlers.bot.reply_to')
    mocker.patch('csuibot.handlers.try_answer', return_value=error_msg)
    mocker.patch('csuibot.handlers.judge_answer', return_value=error_msg)
    mock_message = Mock(text="C")
