...
class Acronym:
    ...
    def add_acronym(self, message):
        ...
        if acronym_dict['singkatan'] in data:
            ...
            file_append = open("csuibot/utils/acronym.json", 'w', encoding='utf-8')
            entry = {acronym_dict['singkatan']:
                     {"singkatan": acronym_dict['singkatan'],
                      "acronym": acronym_dict['acronym']}}
            data.update(entry)
